 export default[
 { 
  // Verificação de status de Hotel Aguardando confirmação de pagamento - Boleto 
    order: {
      type: {
        label: 'Hotel',
  ...
  },
  {
    ...
    // Verificação de status de Hotel Aguardando confirmação de pagamento - Pix
    order: {
      type: {
        label: 'Hotel',
   ...
   //Inicio pedido 3 - VERIFICAÇÃO STATUS HOTEL


   //Inicio pedido 4 - VERIFICAÇÃO STATUS HOTEL
   // Verificação de status de Hotel Pagamento recusado PIX
   {
    order: {
    type: {
      label: 'Hotel',
    ...
    status: {
      label: 'Pagamento recusado - PIX',
      value: 2 // 1(AGUARDANDO), 2 (ERRO, CANCELADO), 3 (FINALIZADA),
    },
    destination: 'Gran Hotel Stella Maris Urban Resort & Conventions',
    localization: 'Salvador, Bahia, Brasil', //Opcional (Somente Hotel)
    order_identifier: 'HKL-8X0-25',
...
hotel: {
 ...
 //FIM PEDIDO 4 - TESTE VERIFICAÇÃO STATUS HOTEL 
  //Inicio pedido 4 - VERIFICAÇÃO STATUS HOTEL
   // Verificação de status de Hotel Pagamento recusado PIX
   {
    order: {
...
hotel: {
 ...
 //FIM PEDIDO 5 - TESTE VERIFICAÇÃO STATUS HOTEL  //Inicio pedido 4 - VERIFICAÇÃO STATUS HOTEL



   // Verificação de status de Hotel Pagamento confirmado
   {
    order: {
    type: {
      label: 'Hotel',
...
hotel: {
 ...
 //FIM PEDIDO 5 - TESTE VERIFICAÇÃO STATUS HOTEL 
 
 
//FIM PEDIDO 6 - TESTE VERIFICAÇÃO STATUS HOTEL 
...
hotel: {
 ...
 //FIM PEDIDO 7 - TESTE VERIFICAÇÃO STATUS HOTEL 
...
hotel: {
 ...
 //FIM PEDIDO 7 - TESTE VERIFICAÇÃO STATUS HOTEL 
  ...
  //FIM PEDIDO 8 - TESTE VERIFICAÇÃO STATUS HOTEL 
  ...
  //inicio pedido 4
{
  order: {
      type: {
        label: 'Voo',
        value: 'V' //V (VOO), H (HOTEL), P(PACOTE), Não exites(O (ONIBUS), C (CARRO))
      },
      status: {
        label: 'aguardando confrimação de pagamento',
        value: 1 // 1(AGUARDANDO), 2 (ERRO, CANCELADO), 3 (FINALIZADA),
      },
      destination: '',
      localization: '', //Opcional (Somente Hotel)
      order_identifier: 'WNX-8X0-19',
      payment_method: 'Boleto ',
      created_at: '2023-01-01',
      total_price: 1093.1,
  ...
  { 
    // Verificação do layout faltando secções(exclua rooms)
      order: {
        type: {
          label: 'Hotel',
          value: 'Hotel' //V (VOO), H (HOTEL), P(PACOTE), Não exites(O (ONIBUS), C (CARRO))
    ...
    // finalização(exclua rooms)

    //Verificação do layout faltando secções(exclua o conection)
    ...
    { 
      //Verificação do layout faltando secções(exclua o conection)
        order: {
          type: {
            label: 'Pacote',
            value: 'P' //V (VOO), H (HOTEL), P(PACOTE), Não exites(O (ONIBUS), C (CARRO))
          },
          status: {
            label: 'Aguardando confirmação de pagamento',
            value: 1 // 1(AGUARDANDO), 2 (ERRO, CANCELADO), 3 (FINALIZADA),
        ...
        // final (exclua conection)
      { 
        //Verificação do layout faltando secções(exclua hotel)
          order: {
            type: {
              label: 'Pacote',
              value: 'P' //V (VOO), H (HOTEL), P(PACOTE), Não exites(O (ONIBUS), C (CARRO))
